...
async function secureInvoke(channel: string, ...args: any[]) {
  try {
    ...
    if (
      ...
      err.message.includes("NOT_ACTIVATED")
    ) {
      // @ts-ignore
      window.dispatchEvent(
        new CustomEvent("novo:auth_error", { detail: err.message }),
      );
    } else if (err.message.includes("NETWORK_UNAVAILABLE")) {
      // @ts-ignore
      window.dispatchEvent(
        new CustomEvent("novo:network_error", { detail: err.message }),
      );
    }
...
const api = {
  ...
  activation: {
    ...
    registerKey: (activationCode: string) =>
      secureInvoke("api:activation:registerKey", activationCode),
  ...
  product: {
    ...
    update: (id: number, payload: any) =>
      secureInvoke("api:product:update", { id, payload }),
    search: (query: string) => secureInvoke("api:product:search", query),
    setActive: (id: number, active: boolean) =>
      secureInvoke("api:product:setActive", { id, active }),
  },
  medicineDirectory: {
    search: (query: string) =>
      secureInvoke("api:medicineDirectory:search", query),
  ...
  supplier: {
    ...
    update: (id: number, payload: any) =>
      secureInvoke("api:supplier:update", { id, payload }),
    search: (query: string) => secureInvoke("api:supplier:search", query),
    setActive: (id: number, active: boolean) =>
      secureInvoke("api:supplier:setActive", { id, active }),
  },
  purchase: {
    ...
    createDraft: (payload: any) =>
      secureInvoke("api:purchase:createDraft", payload),
    updateDraft: (id: number, payload: any) =>
      secureInvoke("api:purchase:updateDraft", { id, payload }),
  ...
  inventory: {
    ...
    getBatch: (productId: number, batchNumber: string) =>
      secureInvoke("api:inventory:getBatch", { productId, batchNumber }),
    getBatchById: (id: number) =>
      secureInvoke("api:inventory:getBatchById", id),
    updateBatch: (id: number, payload: any) =>
      secureInvoke("api:inventory:updateBatch", { id, payload }),
    getActiveBatches: (productId: number) =>
      secureInvoke("api:inventory:getActiveBatches", productId),
  },
  sale: {
  ...
  purchaseReturn: {
    ...
    create: (payload: any) =>
      secureInvoke("api:purchase-return:create", payload),
  },
  stockAdjustment: {
    list: () => secureInvoke("api:stock-adjustment:list"),
    create: (payload: any) =>
      secureInvoke("api:stock-adjustment:create", payload),
  },
  dashboard: {
  ...
  financial: {
    getSummary: (period: string) =>
      secureInvoke("api:financial:getSummary", period),
  },
  customer: {
    ...
    update: (id: number, payload: any) =>
      secureInvoke("api:customer:update", { id, payload }),
    history: (id: number, page?: number, pageSize?: number) =>
      secureInvoke("api:customer:history", { id, page, pageSize }),
  },
  prescription: {
    list: (customerId: number, page?: number, pageSize?: number) =>
      secureInvoke("api:prescription:list", { customerId, page, pageSize }),
  ...
  settings: {
    ...
    getVersion: async () => {
      try {
        return await ipcRenderer.invoke("api:settings:getVersion");
      } catch {
        return { version: "1.2.2", build: "Production Build" };
      }
    },
  },
  report: {
    sales: (start: number, end: number, page?: number, pageSize?: number) =>
      secureInvoke("api:report:sales", { start, end, page, pageSize }),
    purchases: (start: number, end: number, page?: number, pageSize?: number) =>
      secureInvoke("api:report:purchases", { start, end, page, pageSize }),
    salesReturns: (
      start: number,
      end: number,
      page?: number,
      ...
      secureInvoke("api:report:salesReturns", { start, end, page, pageSize }),
    inventory: (page?: number, pageSize?: number, lowStockOnly?: boolean) =>
      secureInvoke("api:report:inventory", { page, pageSize, lowStockOnly }),
    financials: (start: number, end: number) =>
      secureInvoke("api:report:financials", { start, end }),
    inventoryReport: (
      start: number,
      end: number,
      page?: number,
      ...
      secureInvoke("api:report:inventoryReport", {
        start,
        end,
        page,
        pageSize,
    ...
    medicinesReport: (
      ...
      secureInvoke("api:report:medicinesReport", {
        start,
        end,
        page,
        pageSize,
  ...
  document: {
    exportInvoice: (id: number, format: "PDF" | "PRINT") =>
      secureInvoke("api:document:exportInvoice", { id, format }),
    exportPrescription: (id: number, format: "PDF" | "PRINT") =>
      secureInvoke("api:document:exportPrescription", { id, format }),
    exportReportCsv: (
      type: "SALES" | "FINANCIAL" | "INVENTORY" | "PURCHASES" | "MEDICINES",
      start: number,
      end: number,
    ) => secureInvoke("api:document:exportReportCsv", { type, start, end }),
  },
  database: {
    selectBackupLocation: () =>
      secureInvoke("api:database:selectBackupLocation"),
    runManualBackup: () => secureInvoke("api:database:runManualBackup"),
    openBackupFolder: (path: string) =>
      secureInvoke("api:database:openBackupFolder", path),
  ...
  update: {
    ...
    onStateChange: (callback: (data: any) => void) => {
      ipcRenderer.on("api:update:onStateChange", (_event, data) =>
        callback(data),
      );
    },
  ...
  print: {
    ...
    printDocument: (html: string, options: any) =>
      secureInvoke("api:print:printDocument", { html, options }),
  },
};
