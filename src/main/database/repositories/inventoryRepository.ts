...
export class InventoryRepository {
  static getBatch(productId: number, batchNumber: string): InventoryBatch | undefined {
    const db = DatabaseManager.getInstance()
    const stmt = db.prepare(`SELECT * FROM inventory_batches WHERE product_id = ? AND batch_number = ?`)
  ...
  static getPaginatedBatches(options: PaginationOptions): PaginatedResult<InventoryBatch> {
    ...
    if (options.sortBy) {
      ...
      else if (options.sortBy === 'expiry_date') orderBy = `ORDER BY b.expiry_date ${dir}`
      else if (options.sortBy === 'quantity') orderBy = `ORDER BY b.quantity ${dir}`
  ...
  static getSummary(): { totalProducts: number, activeBatches: number, expiringSoon: number, expired: number } {
    ...
    const totalProducts = (db.prepare(`SELECT COUNT(id) as count FROM products`).get() as { count: number }).count
    const activeBatches = (db.prepare(`SELECT COUNT(id) as count FROM inventory_batches WHERE quantity > 0`).get() as { count: number }).count
    const expiringSoon = (db.prepare(`SELECT COUNT(id) as count FROM inventory_batches WHERE quantity > 0 AND expiry_date >= ? AND expiry_date < ?`).get(now, now + ninetyDays) as { count: number }).count
    const expired = (db.prepare(`SELECT COUNT(id) as count FROM inventory_batches WHERE quantity > 0 AND expiry_date < ?`).get(now) as { count: number }).count
  ...
  static listAll(): InventoryBatch[] {
    const db = DatabaseManager.getInstance()
    const stmt = db.prepare(`SELECT * FROM inventory_batches ORDER BY expiry_date ASC`)
