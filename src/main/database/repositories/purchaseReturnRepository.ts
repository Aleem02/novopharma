...
export class PurchaseReturnRepository {
  static findById(id: number): PurchaseReturn | undefined {
    ...
    const pr = db.prepare('SELECT * FROM purchase_returns WHERE id = ?').get(id) as PurchaseReturn | undefined
  ...
  static findByReturnNumber(returnNumber: string): PurchaseReturn | undefined {
    const db = DatabaseManager.getInstance()
    const row = db.prepare('SELECT id FROM purchase_returns WHERE return_number = ?').get(returnNumber) as { id: number } | undefined
  ...
  static list(): PurchaseReturn[] {
    const db = DatabaseManager.getInstance()
    const list = db.prepare('SELECT * FROM purchase_returns ORDER BY return_date DESC').all() as PurchaseReturn[]
