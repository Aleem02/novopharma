...
export class SalesReturnRepository {
  static findById(id: number): SalesReturn | undefined {
    ...
    const salesReturn = db.prepare('SELECT * FROM sales_returns WHERE id = ?').get(id) as SalesReturn | undefined
  ...
  static findByReturnNumber(returnNumber: string): SalesReturn | undefined {
    const db = DatabaseManager.getInstance()
    const row = db.prepare('SELECT id FROM sales_returns WHERE return_number = ?').get(returnNumber) as { id: number } | undefined
  ...
  static list(): SalesReturn[] {
    const db = DatabaseManager.getInstance()
    const list = db.prepare('SELECT * FROM sales_returns ORDER BY return_date DESC').all() as SalesReturn[]
