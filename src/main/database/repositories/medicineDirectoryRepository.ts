...
export class MedicineDirectoryRepository {
  static getCount(): number {
    ...
    const row = db.prepare('SELECT COUNT(*) as count FROM medicine_directory').get() as { count: number }
    return row?.count || 0
  }
    ...
    const insertMany = db.transaction((rows: MedicineDirectoryRecord[]) => {
      for (const row of rows) {
        stmt.run(
          ...
          row.unit || null,
          row.pack_type || null,
          row.units_per_pack !== undefined && row.units_per_pack !== null ? row.units_per_pack : null,
