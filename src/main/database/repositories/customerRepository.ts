...
import { Customer, Sale, PaginationOptions, PaginatedResult } from '../../../shared/types'
...
export class CustomerRepository {
  ...
  static getPaginatedCustomers(options: PaginationOptions): PaginatedResult<Customer> {
    const db = DatabaseManager.getInstance()
    ...
    if (options.search) {
      dataSql += ' ORDER BY CASE WHEN name LIKE ? THEN 1 WHEN name LIKE ? THEN 2 ELSE 3 END ASC, created_at DESC LIMIT ? OFFSET ?'
      dataParams.push(`${options.search}%`, `% ${options.search}%`)
    } else {
  ...
  static getCustomerSales(customerId: number, page: number = 1, pageSize: number = 50): { data: Sale[], total: number } {
    const db = DatabaseManager.getInstance()
    const offset = (page - 1) * pageSize

    const countRow = db.prepare('SELECT count(*) as count FROM sales WHERE customer_id = ?').get(customerId) as { count: number }
    const total = countRow.count

    const data = db.prepare('SELECT * FROM sales WHERE customer_id = ? ORDER BY sale_date DESC LIMIT ? OFFSET ?').all(customerId, pageSize, offset) as Sale[]
