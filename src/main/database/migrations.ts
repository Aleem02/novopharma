...
export class MigrationRunner {
  static run(db: Database): void {
    ...
    for (const m of MIGRATIONS) {
      if (ids.has(m.id)) {
        throw new Error(`FATAL: Duplicate migration identifier detected: ${m.id}`)
      }
      ids.add(m.id)
    }
    ...
    const currentVersionRow = db.prepare('SELECT MAX(id) as version FROM _migrations').get() as { version: number | null }
    const currentVersion = currentVersionRow?.version || 0

    Logger.info('Database', `Current schema version: ${currentVersion}`)

    const pendingMigrations = MIGRATIONS.filter(m => m.id > currentVersion).sort((a, b) => a.id - b.id)
    ...
    for (const migration of pendingMigrations) {
      Logger.info('Database', `Applying migration ${migration.id}: ${migration.name}`)
      
      const requiresNoTransaction = migration.sql.includes('-- NO_TRANSACTION');
      
      const runTransactionalMigration = db.transaction(() => {
        db.exec(migration.sql)
        db.prepare('INSERT INTO _migrations (id, name, applied_at) VALUES (?, ?, ?)').run(migration.id, migration.name, Date.now())
      })
      ...
      try {
        if (requiresNoTransaction) {
          Logger.info('Database', `Migration ${migration.id} is marked as NO_TRANSACTION. Executing without transaction wrapper.`)
          runNonTransactionalMigration()
        } else {
          runTransactionalMigration()
        }
        Logger.info('Database', `Migration ${migration.id} applied successfully.`)
      } catch (error: any) {
        if (error.message.includes('duplicate column name')) {
          Logger.warn('Database', `Migration ${migration.id} duplicate column ignored. Force marking as applied.`)
          db.prepare('INSERT OR IGNORE INTO _migrations (id, name, applied_at) VALUES (?, ?, ?)').run(migration.id, migration.name, Date.now())
        } else {
          Logger.error('Database', `Migration ${migration.id} failed, rolled back. Database remains in previous state.`, { error: error.message })
