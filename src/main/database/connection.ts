...
export class DatabaseManager {
  ...
  static initialize(testDbPath?: string): Database.Database {
    if (this.instance) {
      Logger.warn(
        "Database",
        "DatabaseManager.initialize called but connection already exists",
      );
    ...
    if (testDbPath) {
      ...
      if (process.env.NODE_ENV === "test" || process.env.VITEST) {
        throw new Error(
          "FATAL: Attempted to open production database during automated tests.",
        );
      }
    ...
    try {
      ...
      Logger.info("Database", "SQLite connection initialized successfully", {
        path: this.dbPath,
      });
      return this.instance;
    } catch (error: any) {
      Logger.error("Database", "Failed to initialize SQLite connection", {
        error: error.message,
      });
