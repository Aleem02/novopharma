  ...
  static check(db: Database): void {
    try {
      ...
      if (integrityResult.integrity_check !== 'ok') {
        Logger.error('Database', 'SQLite PRAGMA integrity_check failed', integrityResult)
        throw new Error('Database corruption detected via PRAGMA integrity_check.')
      }

      // 2. PRAGMA foreign_key_check
      const fkResults = db.prepare('PRAGMA foreign_key_check;').all()
      if (fkResults.length > 0) {
        Logger.error('Database', 'SQLite PRAGMA foreign_key_check failed', { violations: fkResults.length })
      ...
      Logger.error('Database', 'Integrity check encountered a fatal error', { error: error.message })
      throw error
    }
  }
}
