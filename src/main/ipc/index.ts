...
export function setupIpcHandlers() {
  ipcMain.handle(
    ...
    async (
      ...
      if (url) {
        if (app.isPackaged) {
          isTrusted =
            url.startsWith("file://") && url.includes("renderer/index.html");
        } else {
          isTrusted = url.startsWith(
  ...
  ipcMain.handle(
    ...
    async (event: IpcMainInvokeEvent): Promise<ActivationResult> => {
      // Validate Sender
      const url = event.senderFrame?.url;
      let isTrusted = false;
      if (url) {
      ...
      if (!isTrusted) {
        Logger.error("IPC", "Unauthorized sender attempted activation call", {
          url,
        });
        throw new Error("Unauthorized sender");
      }
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (!isTrusted) {
        Logger.error("IPC", "Unauthorized sender attempted registerKey call", {
          url,
        });
        throw new Error("Unauthorized sender");
      }
  ...
  ipcMain.handle(
    ...
    async (event: IpcMainInvokeEvent, options: any = {}) => {
      ...
      if (
        ...
        ["ALL", "ACTIVE", "INACTIVE", "PRESCRIPTION"].includes(options.filter)
      ) {
      ...
      if (
        ...
        ["name", "barcode", "selling_price", "quantity"].includes(
          options.sortBy,
        )
      ) {
        safeOptions.sortBy = options.sortBy;
        safeOptions.sortDirection =
  ...
  ipcMain.handle(
    ...
    async (
      event: IpcMainInvokeEvent,
      { id, payload }: { id: number; payload: unknown },
    ) => {
      await enforceSecureIpc(event, { isMutation: true });
  ...
  ipcMain.handle(
    "api:product:search",
    async (event: IpcMainInvokeEvent, query: unknown) => {
      await enforceSecureIpc(event);
  ...
  ipcMain.handle(
    ...
    async (event: IpcMainInvokeEvent, options: any = {}) => {
      ...
      if (
        typeof options?.filter === "string" &&
        ["ALL", "ACTIVE", "INACTIVE"].includes(options.filter)
      ) {
        safeOptions.filter = options.filter;
      }
      ...
      if (
        typeof options?.sortBy === "string" &&
        ["name", "contact_person", "phone"].includes(options.sortBy)
      ) {
        safeOptions.sortBy = options.sortBy;
        safeOptions.sortDirection =
          options?.sortDirection === "DESC" ? "DESC" : "ASC";
  ...
  ipcMain.handle(
    ...
    async (event: IpcMainInvokeEvent, options: any = {}) => {
      ...
      if (
        typeof options?.filter === "string" &&
        ["ALL", "DRAFT", "COMPLETED", "CANCELLED"].includes(options.filter)
      ) {
        safeOptions.filter = options.filter;
      }
      ...
      if (
        typeof options?.sortBy === "string" &&
        ["invoice_number", "purchase_date", "total_amount"].includes(
          options.sortBy,
        )
      ) {
        safeOptions.sortBy = options.sortBy;
  ...
  ipcMain.handle(
    ...
    async (event: IpcMainInvokeEvent, options: any = {}) => {
      ...
      if (
        typeof options?.filter === "string" &&
        ["ALL", "EXPIRED", "EXPIRING_SOON", "LOW_STOCK"].includes(
          options.filter,
        )
      ) {
        safeOptions.filter = options.filter;
      ...
      if (
        typeof options?.sortBy === "string" &&
        ["name", "expiry_date", "quantity", "mrp"].includes(options.sortBy)
      ) {
        safeOptions.sortBy = options.sortBy;
        safeOptions.sortDirection =
          options?.sortDirection === "DESC" ? "DESC" : "ASC";
  ...
  ipcMain.handle(
    "api:inventory:getBatch",
    async (
      event: IpcMainInvokeEvent,
      { productId, batchNumber }: { productId: number; batchNumber: string },
    ) => {
      await enforceSecureIpc(event);
  ...
  ipcMain.handle(
    "api:inventory:getActiveBatches",
    async (event: IpcMainInvokeEvent, productId: unknown) => {
      await enforceSecureIpc(event);
  ...
  ipcMain.handle(
    ...
    async (event: IpcMainInvokeEvent, options: any = {}) => {
      ...
      if (
        typeof options?.filter === "string" &&
        ["ALL", "COMPLETED", "RETURNED", "CANCELLED", "HELD"].includes(
          options.filter,
        )
      ) {
        safeOptions.filter = options.filter;
      ...
      if (
        typeof options?.sortBy === "string" &&
        ["invoice_number", "sale_date", "total_amount"].includes(options.sortBy)
      ) {
        safeOptions.sortBy = options.sortBy;
        safeOptions.sortDirection =
          options?.sortDirection === "DESC" ? "DESC" : "ASC";
  ...
  // Financial Summary
  ipcMain.handle(
    "api:financial:getSummary",
    async (event: IpcMainInvokeEvent, period: unknown) => {
      await enforceSecureIpc(event);
      if (
        typeof period !== "string" ||
        !["TODAY", "YESTERDAY", "LAST_7_DAYS", "MONTH"].includes(period)
      ) {
  ...
  ipcMain.handle(
    ...
    async (event: IpcMainInvokeEvent, options: any = {}) => {
      ...
      if (
        typeof options?.sortBy === "string" &&
        ["name", "created_at"].includes(options.sortBy)
      ) {
        safeOptions.sortBy = options.sortBy;
        safeOptions.sortDirection =
          options?.sortDirection === "DESC" ? "DESC" : "ASC";
  ...
  ipcMain.handle(
    ...
    async (
      event: IpcMainInvokeEvent,
      { id, page, pageSize }: { id: number; page?: number; pageSize?: number },
    ) => {
      await enforceSecureIpc(event);
  ...
  ipcMain.handle(
    ...
    async (
      ...
      await enforceSecureIpc(event);
      return PrescriptionService.getCustomerPrescriptions(
        customerId,
        page,
        pageSize,
      );
  ...
  ipcMain.handle("api:settings:getAll", async (event: IpcMainInvokeEvent) => {
    ...
    return {
      ...
      build_mode: app.isPackaged ? "Production Build" : "Development Build",
      ...settings,
    };
  });

  ipcMain.handle(
    "api:settings:getVersion",
    async (event: IpcMainInvokeEvent) => {
      await enforceSecureIpc(event);
      return {
        version: app.getVersion(),
        build: app.isPackaged ? "Production Build" : "Development Build",
      };
    },
  );
  ...
  ipcMain.handle(
    "api:report:sales",
    async (event: IpcMainInvokeEvent, { start, end, page, pageSize }: any) => {
      await enforceSecureIpc(event);
  ...
  ipcMain.handle(
    ...
    async (
      event: IpcMainInvokeEvent,
      { page, pageSize, lowStockOnly }: any,
    ) => {
      await enforceSecureIpc(event);
  ...
  ipcMain.handle(
    "api:report:financials",
    async (event: IpcMainInvokeEvent, { start, end }: any) => {
      await enforceSecureIpc(event);
  ...
  // Documents
  ipcMain.handle(
    "api:document:exportInvoice",
    async (event: IpcMainInvokeEvent, { id, format }: any) => {
      await enforceSecureIpc(event);
      await DocumentService.exportInvoice(id, format);
    },
  );
  ipcMain.handle(
    "api:document:exportPrescription",
    async (event: IpcMainInvokeEvent, { id, format }: any) => {
      await enforceSecureIpc(event);
  ...
  ipcMain.handle(
    "api:document:exportReportCsv",
    async (event: IpcMainInvokeEvent, { type, start, end }: any) => {
      await enforceSecureIpc(event);
  ...
  ipcMain.handle(
    "api:database:selectBackupLocation",
    async (event: IpcMainInvokeEvent) => {
      await enforceSecureIpc(event);
  ...
  ipcMain.handle("api:database:restore", async (event: IpcMainInvokeEvent) => {
    ...
    if (!result.canceled && result.filePaths.length > 0) {
      ...
      const confirm = await dialog.showMessageBox(win, {
        ...
        detail:
          "This will overwrite your current database. A safety backup will be created, but you should only proceed if necessary. The application will reload automatically.",
      });
  ...
  ipcMain.handle(
    ...
    async (event: IpcMainInvokeEvent, { html, options }: any) => {
      await enforceSecureIpc(event);
