...
export class InstallationIdentityService {
  ...
  private static ensureSafeStorageAvailable(): void {
    if (!safeStorage.isEncryptionAvailable()) {
      Logger.error(
        "Security",
        "safeStorage is unavailable. The environment cannot securely protect the private key.",
      );
      throw new Error(
        "FATAL: safeStorage (DPAPI) is not available on this machine. Cannot proceed with production identity.",
      );
    }
  }
  ...
  static initializeIdentity(): PublicInstallationMetadata {
    ...
    if (fs.existsSync(identityPath)) {
      Logger.info(
        "Security",
        "Found existing installation identity. Attempting to unlock.",
      );
    ...
    Logger.info(
      "Security",
      "No installation identity found. Generating new cryptographic identity.",
    );
   ...
   */
  private static generateAndStoreIdentity(
    filePath: string,
  ): PublicInstallationMetadata {
    ...
    const encryptedPrivateKeyBuffer =
      safeStorage.encryptString(privateKeyString);
    const encryptedPrivateKeyBase64 =
      encryptedPrivateKeyBuffer.toString("base64");
    ...
    Logger.info(
      "Security",
      `Successfully generated and secured installation identity ${identity.installationId}.`,
    );
   ...
   */
  private static loadAndVerifyIdentity(
    filePath: string,
  ): PublicInstallationMetadata {
    ...
    try {
      ...
      throw new Error(
        "FATAL: Failed to read or parse installation identity file. File may be corrupted.",
      );
    }

    if (
      identity.version !== this.CURRENT_VERSION ||
      identity.algorithm !== "Ed25519"
    ) {
      throw new Error(
        `FATAL: Unsupported identity schema or algorithm (Version: ${identity.version}, Algorithm: ${identity.algorithm}).`,
      );
    }
    ...
    try {
      ...
      throw new Error(
        "FATAL: Failed to decrypt private key. Windows secure storage (DPAPI) rejected the payload or the payload is corrupted.",
      );
    }
    ...
    try {
      ...
      const derivedPublicKeyBase64 = derivedPublicKeyObject
      ...
      throw new Error(
        "FATAL: Cryptographic verification failed. The derived public key does not match the stored public key.",
      );
    }

    Logger.info(
      "Security",
      `Successfully loaded and verified installation identity ${identity.installationId}.`,
    );
  ...
  static markAsActivated(): void {
    ...
    if (!fs.existsSync(identityPath)) {
      throw new Error(
        "FATAL: Cannot mark activated, installation identity is missing.",
      );
    }
    ...
    Logger.info(
      "Security",
      "Successfully updated installation identity as activated.",
    );
  }
  ...
  static setActivationStatus(status: boolean): void {
    ...
    if (!fs.existsSync(identityPath)) {
      throw new Error(
        "FATAL: Cannot update status, installation identity is missing.",
      );
    }
    ...
    Logger.info(
      "Security",
      `Successfully updated installation identity activation status to ${status}.`,
    );
  }
