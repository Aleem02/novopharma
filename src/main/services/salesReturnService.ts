...
import { CreateSalesReturnPayload, SalesReturn, ApiError } from '../../shared/types'
...
export class SalesReturnService {
  static createReturn(payload: CreateSalesReturnPayload): SalesReturn {
    return executeTransaction(DatabaseManager.getInstance(), () => {
      ...
      const lastIdRow = db.prepare('SELECT id FROM sales_returns ORDER BY id DESC LIMIT 1').get() as { id: number } | undefined
      ...
      for (const item of payload.items) {
        ...
        if (!saleItem) {
          throw new ApiError(400, `Sale item ID ${item.sale_item_id} not found on invoice ${sale.invoice_number}`)
        }

        const product = saleItem.product
        const unitsPerPack = product?.units_per_pack || 1
        const requestedReturnBaseQty = item.is_pack ? item.quantity * unitsPerPack : item.quantity
        const enteredReturnQty = item.quantity
        const returnUnit = item.is_pack ? (product?.pack_type || 'Pack') : (product?.unit || 'Unit')

        if (!Number.isInteger(requestedReturnBaseQty) || requestedReturnBaseQty <= 0) {
          throw new ApiError(400, `Return quantity must resolve to a positive integer`)
        }

        const alreadyReturned = SalesReturnRepository.getAlreadyReturnedQuantity(item.sale_item_id)
        ...
        for (const batch of batches) {
           ...
           const batchAlreadyReturnedRow = db.prepare(`
             SELECT SUM(quantity) as total FROM sales_return_items 
             WHERE sale_item_id = ? AND inventory_batch_id = ?
           `).get(saleItem.id, batch.inventory_batch_id) as { total: number | null }
           
           const batchAlreadyReturned = batchAlreadyReturnedRow?.total || 0
           const batchAvailableToReturn = batch.base_quantity - batchAlreadyReturned
           
           if (batchAvailableToReturn <= 0) continue;
           
           const qtyToReturnToThisBatch = Math.min(batchAvailableToReturn, remainingQtyToReturn)
           
           const apportionedDiscount = Math.round((saleItem.discount_amount * qtyToReturnToThisBatch) / saleItem.base_quantity)
           const apportionedTax = Math.round((saleItem.tax_amount * qtyToReturnToThisBatch) / saleItem.base_quantity)
           
           // Use overridden price if applicable, otherwise use the specific batch's selling price
           const effectiveSellingPrice = saleItem.is_price_overridden ? saleItem.selling_price : batch.selling_price
           ...
           if (payload.reason !== 'Damaged / Defective' && payload.reason !== 'Expired Medicine') {
             updateBatchStmt.run(qtyToReturnToThisBatch, now, batch.inventory_batch_id)
           }
        ...
        if (remainingQtyToReturn > 0) {
           throw new ApiError(500, `Failed to fully apportion return to batches for sale item ${saleItem.id}`)
        }
      }
      ...
      for (const si of sale.items || []) {
        ...
        const totalReturnedForItem = SalesReturnRepository.getAlreadyReturnedQuantity(si.id)
      ...
      if (newStatus !== sale.status) {
        db.prepare('UPDATE sales SET status = ?, updated_at = ? WHERE id = ?').run(newStatus, now, sale.id)
      }
