...
export class PurchaseService {
  ...
  static updateDraft(id: number, payload: UpdatePurchasePayload): Purchase {
    return executeTransaction(DatabaseManager.getInstance(), () => {
      ...
      if (payload.items) {
        ...
        for (const item of payload.items) {
          const baseQuantity =
            item.quantity * (item.entered_units_per_pack || 1);
          const sellingPrice =
            item.selling_price !== undefined && item.selling_price !== null
              ? item.selling_price
              : item.mrp || 0;
      ...
      const total_amount = itemsToSave.reduce(
        (sum, item) => sum + item.purchase_price * item.quantity,
        0,
      );
  ...
  static completePurchase(id: number): Purchase {
    ...
    return executeTransaction(DatabaseManager.getInstance(), () => {
      ...
      db.prepare(
        `UPDATE purchases SET status = 'COMPLETED', updated_at = ? WHERE id = ?`,
      ).run(now, id);

      // 2. Accumulate Inventory Batches
      const batchSelectStmt = db.prepare(
      ...
      for (const item of purchase.items) {
        ...
        if (!product)
          throw new ApiError(
            400,
            `Product ${item.product_id} not found during purchase completion`,
          );
        ...
        const existingBatch = batchSelectStmt.get(
          item.product_id,
          item.batch_number,
        );
        if (existingBatch) {
  ...
  private static validateSupplier(supplierId: number) {
    ...
    if (supplier.is_active === 0)
      throw new ApiError(400, "Cannot use an inactive supplier");
  }
  ...
  private static validateItems(
    ...
    for (const item of items) {
      ...
      if (!Number.isInteger(item.purchase_price) || item.purchase_price < 0)
        throw new ApiError(
          400,
          "Purchase price must be a non-negative integer",
        );
      if (!Number.isInteger(item.mrp) || item.mrp < 0)
        throw new ApiError(400, "MRP must be a non-negative integer");
