...
import { DashboardSummary, Sale, Purchase, SalesReturn, StockAdjustment } from '../../shared/types'
...
export class DashboardService {
  static getSummary(): DashboardSummary {
    ...
    const totalProductsRow = db.prepare('SELECT COUNT(id) as count FROM products WHERE is_active = 1').get() as { count: number }
    ...
    const recentSaleIds = db.prepare('SELECT id FROM sales ORDER BY sale_date DESC LIMIT 5').all() as { id: number }[]
    const recentSales = recentSaleIds.map(row => SaleRepository.findById(row.id)!) as Sale[]

    // 7. Recent Purchases (last 5)
    const recentPurchaseIds = db.prepare('SELECT id FROM purchases ORDER BY purchase_date DESC LIMIT 5').all() as { id: number }[]
    const recentPurchases = recentPurchaseIds.map(row => PurchaseRepository.findById(row.id)!) as Purchase[]

    // 8. Recent Returns (last 5)
    const recentReturnIds = db.prepare('SELECT id FROM sales_returns ORDER BY return_date DESC LIMIT 5').all() as { id: number }[]
    const recentReturns = recentReturnIds.map(row => SalesReturnRepository.findById(row.id)!) as SalesReturn[]

    // 9. Recent Adjustments (last 5)
    const recentAdjustmentIds = db.prepare('SELECT id FROM stock_adjustments ORDER BY adjusted_at DESC LIMIT 5').all() as { id: number }[]
    const recentAdjustments = recentAdjustmentIds.map(row => StockAdjustmentRepository.findById(row.id)!) as StockAdjustment[]
