  ...
  static createReturn(payload: CreatePurchaseReturnPayload): PurchaseReturn {
    return executeTransaction(DatabaseManager.getInstance(), () => {
      ...
      if (!purchase) {
        throw new ApiError(
          404,
          `Purchase with ID ${payload.purchase_id} not found`,
        );
      }
      ...
      if (purchase.status !== "COMPLETED") {
        throw new ApiError(
          400,
          `Cannot return items from a non-COMPLETED purchase`,
        );
      }
      ...
      const lastIdRow = db
        .prepare("SELECT id FROM purchase_returns ORDER BY id DESC LIMIT 1")
      ...
      for (const item of payload.items) {
        const purchaseItem = purchase.items?.find(
          (pi) => pi.id === item.purchase_item_id,
        );
        if (!purchaseItem) {
          throw new ApiError(
            400,
            `Purchase item ID ${item.purchase_item_id} not found on invoice`,
          );
        }
        ...
        const dbProduct = db
          .prepare(
            "SELECT units_per_pack, pack_type, unit FROM products WHERE id = ?",
          )
          .get(purchaseItem.product_id) as any;
        const unitsPerPack = dbProduct?.units_per_pack || 1;

        const enteredReturnQty = item.quantity;
        const returnUnit = item.is_pack
          ? dbProduct?.pack_type || "Pack"
          : dbProduct?.unit || "Unit";
        const baseReturnQty = item.is_pack
        ...
        const alreadyReturnedBase =
          PurchaseReturnRepository.getAlreadyReturnedQuantity(
        ...
        const batch = getBatchStmt.get(
          purchaseItem.product_id,
          purchaseItem.batch_number,
        ) as InventoryBatch | undefined;
        if (!batch) {
          throw new ApiError(
            400,
            `Inventory batch not found for product ID ${purchaseItem.product_id} and batch ${purchaseItem.batch_number}`,
          );
        }
        ...
        const originalLineTotal =
          purchaseItem.quantity * purchaseItem.purchase_price;
        const lineTotal = Math.round(
          (originalLineTotal * baseReturnQty) / purchaseItem.base_quantity,
        );
