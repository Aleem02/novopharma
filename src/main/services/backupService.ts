...
export class BackupService {
  ...
  static async createBackup(destinationPath: string): Promise<boolean> {
    try {
      ...
      Logger.info(
        "Backup",
        `Backup created successfully at ${destinationPath}`,
      );
      return true;
    } catch (error: any) {
  ...
  static async restoreBackup(backupPath: string): Promise<boolean> {
    ...
    try {
      ...
      if (fkViolations && fkViolations.length > 0) {
        throw new Error(
          "Backup contains foreign key violations and is unsafe to restore.",
        );
      }

      // Check for expected schema table
      const hasMigrations = testDb
        .prepare(
      ...
      throw new Error(
        `Restore failed: the selected backup failed database validation. Your current data was not changed. (${error.message})`,
      );
    }
    ...
    try {
      ...
      if (db.inTransaction) {
        throw new Error(
          "Cannot restore while a transaction is active. Please try again.",
        );
      }
      ...
      const safetyBackupPath = join(
        userData,
        `safety_backup_${Date.now()}.sqlite`,
      );
      ...
      Logger.error(
        "Backup",
        `Failed to create safety backup: ${error.message}`,
      );
      throw new Error(
        `Failed to create safety backup. Restore aborted to prevent data loss. Error: ${error.message}`,
      );
    }
    ...
    try {
      ...
      const userData = app.getPath("userData");
      const targetDbPath =
        (DatabaseManager as any).dbPath ||
        join(userData, "novopharma_v1.sqlite");
      ...
      Logger.info(
        "Backup",
        `Restore applied to ${targetDbPath}. Reinitializing...`,
      );
      ...
      Logger.error(
        "Backup",
        `CRITICAL: Failed during restore overwrite/reinitialize: ${error.message}`,
      );
...
export class BackupManager {
  ...
  static async executeFullBackupFlow() {
    ...
    try {
      ...
      Logger.info(
        "BackupManager",
        "Local backup successfully completed and validated",
      );
      ...
      if (fullPath && fs.existsSync(fullPath)) {
        try {
          ...
          Logger.info(
            "BackupManager",
            `Cleaned up invalid backup file: ${fullPath}`,
          );
        } catch (cleanupErr) {
          Logger.error(
            "BackupManager",
            "Failed to cleanup invalid backup file",
          );
        }
      }
  ...
  private static enforceRetention(backupDir: string) {
    try {
      ...
      if (backupFiles.length > this.MAX_RETENTION) {
        ...
        for (const f of filesToDelete) {
          try {
            ...
            Logger.error(
              "BackupManager",
              `Failed to delete old backup ${f.file}`,
            );
          }
        }
      }
      ...
      Logger.error(
        "BackupManager",
        `Failed to enforce retention: ${err.message}`,
      );
    }
  }
    ...
    try {
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error("App-close backup timeout exceeded")),
            10000,
          ),
        ),
      ]);
      ...
      Logger.error(
        "BackupManager",
        `App-close backup aborted or failed: ${err.message}`,
      );
    }
  }
}
