...
export class UpdateService {
  ...
  static initialize() {
    ...
    autoUpdater.on('download-progress', (progress) => {
      ...
      if (percent !== this.lastProgressPercent && 
          (percent - this.lastProgressPercent >= 1 || now - this.lastProgressTime >= 500)) {
    ...
    autoUpdater.on('error', (err) => {
      ...
      const isNetworkError = errStr.includes('net::') || 
                             errStr.includes('ENOTFOUND') || 
                             errStr.includes('EAI_AGAIN') || 
                             errStr.includes('offline')

      if (!this.isManual && isNetworkError) {
        Logger.info('UpdateService', 'Suppressing background update check error due to offline/network status.')
        this.setState('IDLE')
      } else {
        this.setState('ERROR', { 
          message: isNetworkError ? 'Network connection unavailable.' : 'Failed to download or apply update.' 
        })
      }
    })
  ...
  static async checkForUpdates(isManual = false): Promise<UpdateCheckResult | null> {
    if (!app.isPackaged) {
    ...
    if (this.currentState === 'CHECKING' || this.currentState === 'DOWNLOADING' || this.currentState === 'DOWNLOADED') {
      return null
    }
    ...
    try {
      ...
      if (!this.isManual && isNetworkError) {
        ...
        this.setState('ERROR', { 
          message: isNetworkError ? 'Network connection unavailable.' : 'Failed to check for updates.' 
        })
      }
      return null
    }
  ...
  static async downloadUpdate(): Promise<any> {
    ...
    if (this.currentState === 'DOWNLOADING' || this.currentState === 'DOWNLOADED') {
      return null
    }
  ...
  static async applyUpdate(): Promise<boolean> {
    ...
    try {
      ...
      this.setState('ERROR', { message: err.message || 'Failed to install update.' })
      throw err
    }
  }
