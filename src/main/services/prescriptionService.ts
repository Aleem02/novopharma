...
export class PrescriptionService {
  static createPrescription(payload: CreatePrescriptionPayload): Prescription {
    return executeTransaction(DatabaseManager.getInstance(), () => {
      ...
      if (!payload.items || payload.items.length === 0) {
        throw new ApiError(
          400,
          "Prescription must contain at least one medicine item",
        );
      }
      ...
      for (const item of payload.items) {
        ...
        if (!nameSnapshot || nameSnapshot.trim() === "") {
          throw new ApiError(
            400,
            "Medicine name snapshot is required for all items",
          );
        }
  ...
  static getCustomerPrescriptions(
    ...
    page: number = 1,
    pageSize: number = 50,
  ): PaginatedResult<Prescription> {
    const { data, total } = PrescriptionRepository.getCustomerPrescriptions(
      customerId,
      page,
      pageSize,
    );
