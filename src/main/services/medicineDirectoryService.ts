...
import { MedicineDirectoryRepository, MedicineDirectoryRecord } from '../database/repositories/medicineDirectoryRepository'
...
export class MedicineDirectoryService {
  ...
  static getDatasetPath(): string {
    ...
    const devPath = join(app.getAppPath(), 'src/renderer/src/assets/medicine_directory.json')
    ...
    throw new Error(`medicine_directory.json dataset not found. Checked devPath: "${devPath}" and prodPath: "${prodPath}"`)
  }
  ...
  static initializeBackgroundImport(): void {
    ...
    Promise.resolve().then(async () => {
      try {
        Logger.info('MedicineDirectory', 'Checking if medicine directory table contains records...')
        const isPopulated = MedicineDirectoryRepository.isPopulated()
        if (isPopulated) {
          const count = MedicineDirectoryRepository.getCount()
          Logger.info('MedicineDirectory', `Medicine directory already populated with ${count} records. State set to READY.`)
          this.state = 'READY'
          return
        }

        Logger.info('MedicineDirectory', 'Medicine directory table is empty. Starting background import...')
        const startTime = Date.now()
        const datasetPath = this.getDatasetPath()
        
        Logger.info('MedicineDirectory', `Reading dataset from: ${datasetPath}`)
        ...
        Logger.info('MedicineDirectory', `Parsed ${records.length} records. Committing bulk insert to SQLite...`)
        MedicineDirectoryRepository.bulkInsert(records)

        const duration = Date.now() - startTime
        Logger.info('MedicineDirectory', `Import completed successfully in ${duration}ms. State set to READY.`)
        this.state = 'READY'
      } catch (error: any) {
        Logger.error('MedicineDirectory', `Background import failed: ${error.message}`, error)
        this.state = 'FAILED'
  ...
  static async search(query: string): Promise<MedicineDirectoryRecord[]> {
    ...
    try {
      ...
      Logger.error('MedicineDirectory', `Search query failed: ${error.message}`)
      return []
    }
  }
}
