...
import { CreateSalePayload, Sale, ApiError, InventoryBatch, PaginationOptions, PaginatedResult } from '../../shared/types'
...
export class SaleService {
    ...
    return executeTransaction(DatabaseManager.getInstance(), () => {
      ...
      if (!payload.payments || payload.payments.length === 0) {
        ...
        if (payload.payment_method) {
           payload.payments = [{ payment_method: payload.payment_method, amount: 0 }] // Amount will be calculated below
        } else {
           throw new ApiError(400, 'At least one payment method is required')
        }
      }
      for (const p of payload.payments) {
        if (p.amount < 0) throw new ApiError(400, 'Payment amounts cannot be negative')
      }
      ...
      const checkInvoice = db.prepare('SELECT id FROM sales WHERE invoice_number = ?').get(invoiceNumber)
      if (checkInvoice) {
        throw new ApiError(400, `Invoice sequence conflict: ${invoiceNumber} already exists. Please verify settings.`)
      }
      ...
      for (const item of payload.items) {
        ...
        if (!product) throw new ApiError(400, `Product ID ${item.product_id} not found`)
        if (product.is_active === 0) throw new ApiError(400, `Product ${product.name} is inactive and cannot be sold`)
        if (!Number.isInteger(item.quantity) || item.quantity <= 0) throw new ApiError(400, `Quantity for ${product.name} must be a positive integer`)
        if (product.selling_price < 0 || product.tax_rate < 0 || product.tax_rate > 10000) {
           throw new ApiError(400, `Invalid financial configuration for product ${product.name}`)
        }

        // Find available unexpired batches (FEFO)
        const batches = getBatchesStmt.all(item.product_id, now) as InventoryBatch[]
        
        const unitsPerPack = product.units_per_pack || 1
        const requestedQuantity = item.is_pack ? item.quantity * unitsPerPack : item.quantity
        const enteredUnit = item.is_pack ? (product.pack_type || 'Pack') : (product.unit || 'Unit')
        const enteredQuantity = item.quantity
        
        if (!Number.isInteger(requestedQuantity) || requestedQuantity <= 0) {
          throw new ApiError(400, `Resolved quantity for ${product.name} must be a positive integer`)
        }
        ...
        for (const batch of batches) {
          ...
          const qtyFromThisBatch = Math.min(batch.quantity, remainingQtyToFulfill)
          
          const actualSellingPrice = batch.selling_price > 0 ? batch.selling_price : product.selling_price
        ...
        if (remainingQtyToFulfill > 0) {
          throw new ApiError(400, `Insufficient stock for ${product.name}. Requested: ${requestedQuantity}, Available: ${requestedQuantity - remainingQtyToFulfill}`)
        }

        // Calculate blended prices per base unit
        const blendedSellingPrice = Math.round(blendedSellingPriceSum / requestedQuantity)
        const blendedMrp = Math.round(blendedMrpSum / requestedQuantity)
        
        const originalCommercialSellingPrice = item.is_pack ? blendedSellingPrice * unitsPerPack : blendedSellingPrice
        const commercialMrp = item.is_pack ? blendedMrp * unitsPerPack : blendedMrp
        ...
        if (item.is_price_overridden && item.overridden_price !== undefined) {
          if (item.overridden_price < 0) throw new ApiError(400, `Overridden price for ${product.name} cannot be negative`)
          if (item.overridden_price > commercialMrp) throw new ApiError(400, `Overridden price for ${product.name} cannot exceed MRP of ${commercialMrp}`)
        ...
        if (item.discount_type === 'PERCENTAGE' && item.discount_value) {
           if (item.discount_value > 100 || item.discount_value < 0) throw new ApiError(400, `Invalid percentage discount for ${product.name}`)
           lineDiscountAmount = Math.round((lineGross * item.discount_value) / 100)
        } else if (item.discount_type === 'FIXED' && item.discount_value) {
           if (item.discount_value > lineGross || item.discount_value < 0) throw new ApiError(400, `Invalid fixed discount for ${product.name}`)
        ...
        const lineTaxAmount = Math.round((discountedLineTotal * product.tax_rate) / (10000 + product.tax_rate))
        ...
        saleItemsToInsert.push({
           ...
           selling_price: item.is_price_overridden && item.overridden_price !== undefined 
                           ? Math.round(item.overridden_price / (item.is_pack ? unitsPerPack : 1)) 
                           : blendedSellingPrice,
        ...
        subtotal += (lineGross - Math.round((lineGross * product.tax_rate) / (10000 + product.tax_rate)))
      }
      
      // Calculate Bill Level Discount
      let billDiscountAmount = 0
      if (payload.bill_discount_type === 'PERCENTAGE' && payload.bill_discount_value) {
          if (payload.bill_discount_value > 100 || payload.bill_discount_value < 0) throw new ApiError(400, `Invalid bill percentage discount`)
          billDiscountAmount = Math.round((saleGrossTotal * payload.bill_discount_value) / 100)
      } else if (payload.bill_discount_type === 'FIXED' && payload.bill_discount_value) {
          if (payload.bill_discount_value > saleGrossTotal || payload.bill_discount_value < 0) throw new ApiError(400, `Invalid bill fixed discount`)
          billDiscountAmount = payload.bill_discount_value
      }
      
      totalDiscount = saleItemsToInsert.reduce((sum, item) => sum + item.discount_amount, 0) + billDiscountAmount
      ...
      for (const item of saleItemsToInsert) {
         ...
         if (billDiscountAmount > 0 && saleGrossTotal > 0) {
            apportionedBillDiscount = Math.round((item.line_total / saleGrossTotal) * billDiscountAmount)
         }
         
         const finalTaxableLineAmount = item.line_total - apportionedBillDiscount
         const finalLineTax = Math.round((finalTaxableLineAmount * item.tax_rate) / (10000 + item.tax_rate))
         
         item.tax_amount = finalLineTax
         totalTax += finalLineTax
         subtotal += (item.line_total + item.discount_amount) - Math.round(((item.line_total + item.discount_amount) * item.tax_rate) / (10000 + item.tax_rate))
      }
      
      // Calculate Payments
      let totalReceived = payload.received_amount || 0
      let totalPaymentEntries = payload.payments.reduce((sum, p) => sum + p.amount, 0)
      ...
      if (totalPaymentEntries < grandTotal) {
          throw new ApiError(400, `Insufficient payment. Total: ${grandTotal}, Provided: ${totalPaymentEntries}`)
      }
      
      const changeAmount = payload.change_amount !== undefined ? payload.change_amount : (totalReceived > grandTotal ? totalReceived - grandTotal : 0)
      ...
      const primaryPaymentMethod = payload.payments.length > 0 ? payload.payments[0].payment_method : 'CASH'
      ...
      for (const si of saleItemsToInsert) {
        const result = itemStmt.run(
          saleId, si.product_id, si.entered_quantity, si.sale_unit, si.base_quantity,
          si.selling_price, si.original_selling_price, si.is_price_overridden, si.override_reason, si.mrp,
          si.tax_rate, si.tax_amount, si.discount_type, si.discount_value, si.discount_amount, si.line_total, now
        )
        const saleItemId = result.lastInsertRowid as number

        for (const b of si.batches) {
           batchStmt.run(saleItemId, b.inventory_batch_id, b.batch_number, b.base_quantity, b.selling_price, b.mrp, now)
        }
      }
      ...
      for (const p of payload.payments) {
          if (p.amount > 0) {
              paymentStmt.run(saleId, p.payment_method, p.amount, p.reference_number || null, now)
          }
      }
  ...
  static getSale(id: number): Sale {
    ...
    const db = DatabaseManager.getInstance()
    const payments = db.prepare('SELECT * FROM sale_payments WHERE sale_id = ?').all(id) as any[]
  ...
  static getSaleByInvoiceNumber(invoiceNumber: string): Sale {
    ...
    const db = DatabaseManager.getInstance()
    const payments = db.prepare('SELECT * FROM sale_payments WHERE sale_id = ?').all(sale.id) as any[]
    sale.payments = payments
    return sale
  }
