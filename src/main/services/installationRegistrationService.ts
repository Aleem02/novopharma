...
export class InstallationRegistrationService {
  static async registerKey(
    ...
    if (!(await FirebaseAuthService.isAuthenticated())) {
      Logger.warn(
        "Security",
        "Attempted to register key without being authenticated.",
      );
      return {
        status: "ERROR",
    ...
    if (
      ...
      !activationCode ||
      activationCode.replace(/\s+/g, "").length < 6
    ) {
      Logger.warn(
        "Security",
        "Attempted to register key with invalid activation code format.",
      );
      return {
        status: "ERROR",
    ...
    try {
      ...
      Logger.error(
        "Security",
        `Failed to retrieve installation identity for registration: ${err.message}`,
      );
      return {
        status: "ERROR",
    ...
    try {
      ...
      await ApiClient.request(
        `${backendUrl}/api/desktop/activation/register-key`,
        {
          method: "POST",
          headers: {
      ...
      Logger.info(
        "Security",
        `Successfully registered public key for installation ${identity.installationId}. Status: PENDING_APPROVAL`,
      );
      ...
      Logger.error(
        "Security",
        `Network or API client error during registration: ${err.message}`,
      );
      return {
        status: "ERROR",
        code: "REGISTRATION_FAILED",
        message:
          "Unable to contact the activation server. Please check your internet connection and try again.",
      };
    }
  }
    ...
    switch (status) {
      case 400:
        return {
          status: "ERROR",
          code: "REGISTRATION_FAILED",
          message: "Invalid activation code or registration request.",
        };
      case 401:
        return {
          status: "ERROR",
      ...
      case 403:
        return {
          status: "ERROR",
          code: "REGISTRATION_FAILED",
          message: "This installation cannot be registered.",
        };
      case 404:
        return {
          status: "ERROR",
      ...
      case 409:
        return {
          ...
          message:
            "This activation code or installation has already been used.",
        };
      case 410:
        return {
          status: "ERROR",
