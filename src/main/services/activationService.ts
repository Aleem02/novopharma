...
} from "../security/installationIdentity";
...
export class ActivationService {
  ...
  static async activateInstallation(): Promise<ActivationResult> {
    try {
      ...
      // @ts-ignore
      const backendUrl =
        import.meta.env.VITE_BACKEND_URL || "https://api.novopharma.test";
      ...
      try {
        challengeResponse = await ApiClient.request(
          `${backendUrl}/api/desktop/activation/challenge`,
          {
            method: "POST",
            body: JSON.stringify({
        ...
        return {
          status: "ERROR",
          message: "Unable to activate this installation right now.",
        };
      }
      ...
      if (
        ...
        Logger.error(
          "Activation",
          "Backend returned an invalid challenge payload.",
        );
        return {
          status: "ERROR",
          message: "Unable to activate this installation right now.",
      ...
      try {
        ...
        signatureBuffer =
          InstallationIdentityService.signChallenge(payloadBuffer);
      } catch (err: any) {
        Logger.error(
          "Activation",
      ...
      try {
        completeResponse = await ApiClient.request(
          `${backendUrl}/api/desktop/activation/complete`,
          {
            method: "POST",
            body: JSON.stringify({
        ...
        Logger.error(
          "Activation",
          `Activation completion failed: ${err.message}`,
        );
        return {
          status: "ERROR",
          message: "Unable to activate this installation right now.",
      ...
      if (completeResponse && completeResponse.activated === true) {
        Logger.info(
          "Activation",
          `Successfully activated installation ${identity.installationId}`,
        );
        ...
        return {
          ...
          message:
            ...
            "Unable to activate this installation right now.",
        };
      }
    } catch (error: any) {
      Logger.error(
        "Activation",
        `Unhandled error during activation: ${error.message}`,
      );
      return {
        status: "ERROR",
        message: "Unable to activate this installation right now.",
  ...
  private static mapHttpError(status: number): ActivationResult {
    switch (status) {
      ...
      case 409:
        return {
          status: "ERROR",
          message: "Another computer is already active for this pharmacy.",
        };
      case 410:
        return {
          status: "ERROR",
          message: "Activation request expired. Please try again.",
        };
      case 422:
        return {
          status: "ERROR",
  ...
  static async syncStatusWithBackend(): Promise<void> {
    try {
      ...
      try {
        ...
        Logger.error(
          "Activation",
          "Failed to retrieve local identity for sync",
          err,
        );
        return;
      }
      ...
      try {
        response = await ApiClient.request(
          `${backendUrl}/api/desktop/activation/status`,
          {
            method: "POST",
            body: JSON.stringify({
              installationId: identity.installationId,
        ...
        if (
          ...
          Logger.warn(
            "Activation",
            `Machine status is unauthorized/not found on server (HTTP ${err.status}). Deactivating locally.`,
          );
          InstallationIdentityService.setActivationStatus(false);
        } else {
          Logger.warn(
            "Activation",
            `Failed to contact activation status server: ${err.message}. Preserving cached local status.`,
          );
        }
        return;
      }
      ...
      if (response && typeof response.activated === "boolean") {
        ...
        if (localActivated !== response.activated) {
          Logger.info(
            "Activation",
            `Syncing activation status from backend: ${localActivated} -> ${response.activated}`,
          );
      ...
      Logger.error(
        "Activation",
        `Unexpected error during activation sync: ${error.message}`,
      );
    }
  }
