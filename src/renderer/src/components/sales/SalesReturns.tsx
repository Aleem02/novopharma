...
import { SalesReturn } from '../../../../shared/types'
...
export const SalesReturns: React.FC = () => {
  ...
  const fetchReturns = async () => {
    try {
      setLoading(true)
      const data = await window.api.salesReturn.list()
      setReturns(data)
      setError(null)
    } catch (err: any) {
  ...
  const filteredReturns = returns.filter(ret => {
    ...
    return (
      ...
      `id: ${ret.sale_id}`.toLowerCase().includes(query) ||
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500">Track and issue refunds for customer invoices.</p>
        </div>
        <Link
      ...
      <div className="flex justify-between items-center mb-6 bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
        <Input
          ...
          placeholder="Search by Return ID, Invoice Ref, or Reason..."
          value={searchQuery}
      ...
      {returns.length === 0 ? (
        <div className="bg-white border border-slate-100 p-12 text-center rounded-xl shadow-sm">
          ...
          <h3 className="font-bold text-slate-700">No Returns Found</h3>
          <p className="text-sm text-slate-400 mt-1">Start by creating a new customer return.</p>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 text-sm font-medium">
              {filteredReturns.map((ret) => (
                <tr key={ret.id} className="hover:bg-slate-50/50">
                  ...
                  <td className="px-6 py-4 text-slate-600">ID: {ret.sale_id}</td>
                  ...
                  <td className="px-6 py-4 text-right font-bold text-teal-600">{formatPaise(ret.refund_amount)}</td>
                </tr>
              ))}
            </tbody>
          </table>
