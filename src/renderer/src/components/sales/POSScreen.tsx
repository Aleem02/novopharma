...
export const POSScreen: React.FC = () => {
  ...
  const [payments, setPayments] = useState<SalePayment[]>([{ payment_method: 'CASH', amount: 0 }])
        ...
        try {
          ...
          if (product && product.is_active) {
            ...
            setCart(prev => {
              ...
              return [...prev, { product, quantity: item.quantity, is_pack: false }]
            })
            productsAdded++
          } else {
            missingProducts++
  ...
  const addToCart = async (product: Product) => {
    ...
    try {
      const activeBatches = await window.api.inventory.getActiveBatches(product.id)
      ...
      setCart(prev => {
        ...
        if (existing) {
          ...
          return [...prev, { product, quantity: 1, is_pack: false, active_batches: activeBatches, discount_type: 'PERCENTAGE', discount_value: 0, is_price_overridden: false }]
        }
      })
  ...
  const updateDiscount = (productId: number, type: DiscountType, value: number) => {
    setCart(prev => prev.map(item => {
      if (item.product.id === productId) {
  ...
  const updateOverride = (productId: number, overriddenPrice: number | '') => {
    setCart(prev => prev.map(item => {
      if (item.product.id === productId) {
        if (overriddenPrice === '') {
           return { ...item, is_price_overridden: false, overridden_price: undefined }
        } else {
           return { ...item, is_price_overridden: true, overridden_price: Math.round(Number(overriddenPrice) * 100) }
        }
      }
      return item
    }))
  ...
  const handleHoldSale = () => {
    if (cart.length > 0) {
      ...
      clearCustomer()
      setError('Cart held successfully. Press F2 to restore it, or F1 to start a new sale.')
    } else {
      const held = localStorage.getItem('novo_held_cart')
      if (held) {
  ...
  const handleCheckout = async () => {
    ...
    // Check prescriptions
    const needsPrescription = cart.some(item => item.product.prescription_required)
    if (needsPrescription && !selectedPrescriptionId && !window.confirm('Some items require a prescription. Do you want to proceed without attaching one (Pharmacist Override)?')) {
      return;
    }
    ...
    try {
      // Clean payments
      const finalPayments = payments.map(p => ({ ...p, amount: Math.round(Number(p.amount) * 100) })).filter(p => p.amount > 0)
      ...
      const payload = {
        payment_method: finalPayments.length > 0 ? finalPayments[0].payment_method : 'CASH',
        payments: finalPayments,
        received_amount: receivedAmount ? Math.round(Number(receivedAmount) * 100) : undefined,
        bill_discount_type: billDiscountType === 'NONE' ? undefined : billDiscountType,
        bill_discount_value: billDiscountValue ? Math.round(Number(billDiscountValue) * (billDiscountType === 'FIXED' ? 100 : 1)) : undefined,
        ...
        items: cart.map(item => ({
          ...
          discount_type: item.discount_type === 'NONE' ? undefined : item.discount_type,
          discount_value: item.discount_value ? (item.discount_type === 'FIXED' ? Math.round(item.discount_value * 100) : item.discount_value) : undefined,
      ...
      localStorage.removeItem('novo_held_cart') // clear held cart if checkout succeeds
      navigate(`/sales/invoice/${sale.invoice_number}`, { state: { isNewSale: true } })
    } catch (err: any) {
  ...
  const getCartItemBasePrices = (item: POSCartItem) => {
    const originalSellingPrice = item.active_batches && item.active_batches.length > 0
      ? (item.active_batches[0].selling_price > 0 ? item.active_batches[0].selling_price : item.product.selling_price)
      : item.product.selling_price;
    const mrp = item.active_batches && item.active_batches.length > 0
      ? (item.active_batches[0].mrp > 0 ? item.active_batches[0].mrp : item.product.selling_price)
      : item.product.selling_price;
    ...
    return {
       ...
       rate: item.is_price_overridden && item.overridden_price !== undefined ? item.overridden_price : (originalSellingPrice * multiplier)
    }
  }
  ...
  if (billDiscountType === 'PERCENTAGE' && billDiscountValue) {
      billDiscountAmt = Math.round((cartGrossTaxable * Number(billDiscountValue)) / 100)
  ...
  enhancedCart.forEach(item => {
     ...
     if (billDiscountAmt > 0 && cartGrossTaxable > 0) {
        apportionedBillDiscount = Math.round((item.lineTotalAfterDiscount / cartGrossTaxable) * billDiscountAmt)
     }
     const finalTaxable = item.lineTotalAfterDiscount - apportionedBillDiscount
     const tax = Math.round((finalTaxable * item.product.tax_rate) / (10000 + item.product.tax_rate))
     cartTotalTax += tax
  })
  ...
  }, [cart, isProcessing, payments, selectedCustomer, selectedPrescriptionId, billDiscountType, billDiscountValue, receivedAmount])
  ...
  const paymentTotal = payments.reduce((sum, p) => sum + Math.round(Number(p.amount) * 100), 0)
    ...
    <div className="font-sans max-w-[1600px] mx-auto h-full flex flex-col space-y-4">
      ...
      <div className="flex justify-between items-center bg-white p-3 rounded-sm border border-slate-300 shrink-0">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900 leading-none">Point of Sale</h1>
            <p className="text-[11px] text-slate-500 mt-1 uppercase tracking-wider font-semibold">Quick and easy billing for your customers</p>
          </div>
        </div>
        <div className="flex items-center space-x-2">
          <Button variant="outline" className="text-amber-700 border-amber-200 bg-amber-50 text-xs font-bold" onClick={handleHoldSale}>
            <svg className="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />
            </svg>
            F2 - Hold Sale
          </Button>
          <Button variant="outline" className="text-teal-700 border-teal-200 bg-teal-50 text-xs font-bold" onClick={() => { setCart([]); clearCustomer(); }}>
            <svg className="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-3 min-h-0">
        ...
        <div className="lg:col-span-8 flex flex-col bg-white rounded-sm border border-slate-300 overflow-hidden min-h-0">
                    ...
                    >
                      <div>
                        <div className="font-bold text-slate-800 text-sm flex items-center gap-2">
                          {product.name}
                          {product.prescription_required === 1 && <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded uppercase">Rx</span>}
                          {!product.is_active && <span className="text-[10px] bg-red-100 text-red-600 px-2 py-0.5 rounded uppercase">Inactive</span>}
                        </div>
                        <div className="text-xs text-slate-500 mt-1">{product.generic_name} • {product.strength}</div>
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-teal-700">₹{(product.selling_price / 100).toFixed(2)}</div>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="flex-1 overflow-y-auto bg-white">
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-100 text-xs font-semibold text-slate-600 sticky top-0 border-b border-slate-300 z-10 shadow-sm">
                <tr>
                  <th className="px-3 py-2 font-semibold w-8 text-center border-r border-slate-200">#</th>
                  <th className="px-3 py-2 font-semibold border-r border-slate-200 min-w-[150px]">Medicine</th>
                  <th className="px-3 py-2 font-semibold border-r border-slate-200 w-24">Unit</th>
                  <th className="px-3 py-2 font-semibold text-right border-r border-slate-200 w-20">MRP</th>
                  <th className="px-3 py-2 font-semibold text-right border-r border-slate-200 w-24">Rate</th>
                  <th className="px-3 py-2 font-semibold text-center border-r border-slate-200 w-28">Qty</th>
                  <th className="px-3 py-2 font-semibold text-center border-r border-slate-200 w-28">Discount</th>
                  <th className="px-3 py-2 font-semibold text-right border-r border-slate-200 w-24">Total</th>
                  <th className="px-3 py-2 font-semibold text-center w-10">Act</th>
                </tr>
              </thead>
                  ...
                  enhancedCart.map((item, index) => (
                    <tr key={item.product.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-3 py-3">
                         ...
                         {item.product.prescription_required === 1 && <span className="text-[9px] bg-amber-100 text-amber-700 px-1 py-0.5 rounded uppercase inline-block mt-1">Rx Required</span>}
                      </td>
                      <td className="px-2 py-3">
                        {item.product.pack_type && (item.product.units_per_pack || 0) > 1 ? (
                          <select 
                          ...
                          >
                            <option value="unit">{item.product.unit || 'Unit'}</option>
                            <option value="pack">{item.product.pack_type}</option>
                          </select>
                        ) : (
                          <span className="text-slate-500">{item.product.unit || 'Unit'}</span>
                        )}
                      </td>
                         ...
                         <div className="relative group">
                            {item.is_price_overridden ? (
                               <div className="flex flex-col items-end">
                                 <span className="text-[10px] line-through text-slate-400">₹{(item.prices.originalRate / 100).toFixed(2)}</span>
                                 <Input 
                                   type="number" 
                                   className="w-16 h-7 text-right px-1 py-0 text-xs font-bold text-amber-700 bg-amber-50 border-amber-200" 
                                   value={item.overridden_price !== undefined ? (item.overridden_price / 100) : ''}
                                   onChange={e => updateOverride(item.product.id, e.target.value === '' ? '' : Number(e.target.value))}
                                   placeholder="0.00"
                                 />
                               </div>
                            ) : (
                               <div 
                                 ...
                                 onClick={() => updateOverride(item.product.id, (item.prices.originalRate / 100))}
                      ...
                      <td className="px-2 py-3">
                        <div className="flex items-center justify-center">
                          <button onClick={() => updateQuantity(item.product.id, -1)} className="w-6 h-6 flex items-center justify-center border border-slate-200 rounded-l text-slate-500 hover:bg-slate-100">-</button>
                          <input 
                             type="number" 
                             ...
                             onChange={(e) => {
                                ...
                                if (val > 0) {
                                   setCart(prev => prev.map(p => p.product.id === item.product.id ? {...p, quantity: val} : p))
                                }
                             }} placeholder="0"
                          />
                          <button onClick={() => updateQuantity(item.product.id, 1)} className="w-6 h-6 flex items-center justify-center border border-slate-200 rounded-r text-slate-500 hover:bg-slate-100">+</button>
                        </div>
                      </td>
                      <td className="px-2 py-3">
                         <div className="flex border border-slate-300 rounded overflow-hidden h-7">
                           <select 
                             ...
                             onChange={(e) => updateDiscount(item.product.id, e.target.value as DiscountType, item.discount_value || 0)}
                           >
                           ...
                           <input 
                             ...
                             onChange={(e) => updateDiscount(item.product.id, item.discount_type || 'NONE', parseFloat(e.target.value) || 0)} placeholder="0"
                           />
                         </div>
                      </td>
                      ...
                      <td className="px-2 py-3 text-center">
                        <button onClick={() => removeFromCart(item.product.id)} className="text-red-400 hover:text-red-600 p-1 hover:bg-red-50 rounded transition-colors">
          ...
          <div className="p-3 border-t border-slate-200 bg-slate-50 flex items-center justify-center space-x-6 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
             <div className="flex items-center"><span className="bg-white border border-slate-300 px-2 py-0.5 rounded text-slate-700 mr-2 shadow-sm">F1</span> New</div>
             <div className="flex items-center"><span className="bg-white border border-slate-300 px-2 py-0.5 rounded text-slate-700 mr-2 shadow-sm">F2</span> Hold</div>
             <div className="flex items-center"><span className="bg-white border border-slate-300 px-2 py-0.5 rounded text-slate-700 mr-2 shadow-sm">F8</span> Complete</div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 flex flex-col space-y-4 min-h-0">
           ...
           <div className="bg-white rounded-sm border border-slate-300 flex flex-col shrink-0 relative z-40">
             <div className="p-2 border-b border-slate-200 bg-slate-50/50 flex justify-between items-center">
               ...
               {selectedCustomer && (
                 <button onClick={clearCustomer} className="text-[10px] font-bold text-slate-400 border border-slate-200 bg-white px-2 py-1 rounded hover:text-slate-700 shadow-sm transition-colors uppercase">
                   Clear
                 </button>
               )}
               </div>
             ...
             <div className="p-4">
                 ...
                 <div className="flex justify-between items-center">
                   <div className="space-y-1">
                     <div className="flex items-center text-sm">
                       <span className="w-16 text-slate-500 font-medium">Name</span>
                       <span className="font-bold text-slate-900">{selectedCustomer.name}</span>
                     </div>
                     <div className="flex items-center text-sm">
                       <span className="w-16 text-slate-500 font-medium">Phone</span>
               ...
               {selectedCustomer && (
                 <div className="mt-4 pt-3 border-t border-slate-100">
                   <Button variant="outline" className="w-full text-xs font-bold border-slate-200" onClick={() => setShowPrescriptionModal(true)}>
           ...
           <div className="bg-white rounded-sm border border-slate-300 flex flex-col flex-1 min-h-0 overflow-y-auto">
             ...
             <div className="p-4 flex flex-col space-y-2 text-sm">
               <div className="flex justify-between items-center">
                 ...
                 <span className="font-medium text-slate-800">₹{(cartSubtotal / 100).toFixed(2)}</span>
               </div>
               <div className="flex justify-between items-center text-teal-700">
                 <span className="text-teal-600">Item Discounts</span>
                 <span className="font-medium">- ₹{(cartTotalItemDiscount / 100).toFixed(2)}</span>
               </div>
               ...
               <div className="flex justify-between items-center py-2 border-y border-slate-100 my-1">
                 ...
                 <div className="flex border border-slate-300 rounded overflow-hidden h-7 w-32">
                   ...
                   <input 
                     ...
                     onChange={(e) => setBillDiscountValue(e.target.value === '' ? '' : Number(e.target.value))}
                     placeholder="0"
                   />
                 </div>
               </div>
               ...
               <div className="flex justify-between items-center">
                 ...
                 <span className="font-medium text-slate-800">₹{(cartNetSubtotal / 100).toFixed(2)}</span>
               </div>
               <div className="flex justify-between items-center">
                 <span className="text-slate-500">Total Tax</span>
                 <span className="font-medium text-slate-800">₹{(cartTotalTax / 100).toFixed(2)}</span>
               </div>
               ...
               <div className="flex justify-between items-end pt-3 pb-1">
                 ...
                 <span className="text-3xl font-black text-teal-600 tracking-tight">₹{(cartGrandTotal / 100).toFixed(2)}</span>
               </div>

               {/* Split Payment Section */}
               <div className="mt-4 border border-slate-200 rounded-lg p-3 bg-slate-50">
                 <div className="flex justify-between items-center mb-3">
                   <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Payment</h4>
                   <div className="flex items-center space-x-3">
                     <label className="flex items-center space-x-1.5 cursor-pointer bg-teal-50 px-2 py-1 rounded border border-teal-100 hover:bg-teal-100 transition-colors">
                       <input 
                         ...
                         checked={payments.length === 1 && Number(payments[0].amount) === cartGrandTotal / 100}
                         onChange={(e) => {
                           if (e.target.checked) {
                             setPayments([{ payment_method: payments[0]?.payment_method || 'CASH', amount: cartGrandTotal / 100 }])
                           } else {
                             setPayments([{ payment_method: payments[0]?.payment_method || 'CASH', amount: 0 }])
                           }
                         }} placeholder="Enter value..."
                       />
                       <span className="text-[10px] text-teal-700 font-bold uppercase">Paid in Full</span>
                     </label>
                     <button onClick={handleAddPayment} className="text-[10px] bg-white border border-slate-300 px-2 py-1 rounded text-slate-600 hover:bg-slate-100">+ Add Split</button>
                   </div>
                 </div>
                 ...
                 {payments.map((p, index) => (
                   <div key={index} className="flex space-x-2 mb-2 items-center">
                     ...
                     />
                     {payments.length > 1 && (
                       <button onClick={() => removePayment(index)} className="text-red-400 p-1"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
                     )}
                   </div>
                 ))}
                 ...
                 <div className="flex justify-between text-xs mt-3 pt-2 border-t border-slate-200">
                    ...
                    <span className={`font-bold ${!isPaymentValid ? 'text-red-500' : 'text-slate-700'}`}>₹{(paymentTotal / 100).toFixed(2)}</span>
                 </div>
                 ...
                 {payments.some(p => p.payment_method === 'CASH') && (
                   <div className="flex justify-between items-center text-xs mt-2 bg-amber-50 p-2 rounded border border-amber-100">
                      <span className="text-amber-800 font-medium">Cash Received</span>
                      <input 
                         type="number"
               ...
               <Button
                 ...
                 disabled={cart.length === 0 || (!isPaymentValid && paymentTotal > 0)}
               ...
               >
                 ...
                 <span className="text-[10px] bg-white/20 px-1.5 py-0.5 rounded text-teal-50">(F8)</span>
               </Button>
             </div>
           </div>
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          ...
                          <div className="text-xs text-slate-500 font-medium">{new Date(p.prescription_date).toLocaleDateString()} {p.reference_number && `• Ref: ${p.reference_number}`}</div>
                        </div>
                        <Button size="sm" variant="outline" className="border-teal-200 text-teal-700 bg-teal-50">Select & Load</Button>
                      </div>
                      <div className="bg-slate-50 border border-slate-100 rounded-lg p-3 text-sm">
                        <ul className="space-y-2">
                          {p.items?.map(item => (
                            <li key={item.id} className="flex justify-between text-slate-700 items-center">
                              <span className="font-medium">• {item.medicine_name_snapshot} <span className="text-slate-400 text-xs ml-1 font-normal bg-white px-1.5 py-0.5 rounded border border-slate-200">{item.dosage_instructions}</span></span>
                              <span className="font-bold text-slate-800 bg-slate-200/50 px-2 py-0.5 rounded text-xs">Qty: {item.quantity}</span>
                            </li>
                          ))}
                        </ul>
                      </div>
