...
function generateMockToken(installationId, mutate = null) {
    ...
    let signatureBuffer = crypto.sign(undefined, Buffer.from(payloadBase64, 'utf8'), { key: DEV_PRIVATE_KEY, format: 'pem', type: 'pkcs8' });
...
async function runTests() {
    ...
    try {
        ...
        const { InstallationIdentityService } = require('../dist/main/security/installationIdentity.js');
        const { ActivationService } = require('../dist/main/services/activationService.js');
        ...
        assert(!InstallationIdentityService.isActivated(), "1. Fresh installation", "Unactivated");

        // 4, 18. Existing activated installation (legacy)
        writeIdentity({ ...baseIdentity, isActivated: true });
        assert(InstallationIdentityService.isActivated() === true, "18. Test existing isActivated=true legacy installation", "Activated");
        
        const migrationState = InstallationIdentityService.getMigrationState();
        assert(migrationState.isLegacy === true, "4. Existing activated installation enters legacy mode", "Legacy=true");
        ...
        ApiClient.request = async () => {
            return { activated: true, licenseToken: generateMockToken(fakeInstallationId) };
        };
        const { FirebaseAuthService } = require('../dist/main/services/firebaseAuth.js');
        ...
        assert(!!updatedIdentity.licenseToken, "20. Legacy installation when internet becomes available", "Token fetched and saved");

        // 24, 25. Test application after migration completely offline
        assert(InstallationIdentityService.isActivated() === true, "24. Test application after migration", "Activated");
        assert(InstallationIdentityService.getMigrationState().isLegacy === false, "25. Test migrated installation completely offline", "Legacy=false");

        // 21. Invalid licenseToken
        writeIdentity({ ...baseIdentity, licenseToken: "not-a-json-string" });
        assert(!InstallationIdentityService.isActivated(), "21. Test invalid licenseToken", "Fails");

        // 22. Modified licenseToken (tampered signature)
        writeIdentity({ ...baseIdentity, licenseToken: generateMockToken(fakeInstallationId, { tamperSignature: true }) });
        assert(!InstallationIdentityService.isActivated(), "22. Test modified licenseToken", "Fails");

        // 23. Wrong installationId
        writeIdentity({ ...baseIdentity, licenseToken: generateMockToken("wrong-id-here") });
        assert(!InstallationIdentityService.isActivated(), "23. Test wrong installationId", "Fails");

        // 19. Legacy without internet
        writeIdentity({ ...baseIdentity, isActivated: true });
        ApiClient.request = async () => { throw new Error("NETWORK_UNAVAILABLE"); };
        await ActivationService.performLegacyMigration();
        const legacyIdentity = JSON.parse(fs.readFileSync(identityPath, 'utf8'));
        assert(!legacyIdentity.licenseToken && InstallationIdentityService.isActivated(), "19. Test legacy installation with no internet", "Remains active in legacy mode");
