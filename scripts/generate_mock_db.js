...
var runTransaction = db.transaction(function (fn) {
  return fn();
});
runTransaction(function () {
  ...
  var suppliers = [
    [
      ...
      "Rajesh Kumar",
      "9876543210",
      "contact@medicorp.demo",
      "Chennai South",
      "33BBBBB1111B1Z1",
    ],
    [
      "PharmaPlus Wholesale",
      "Anitha S",
      "9876543211",
      "sales@pharmaplus.demo",
      "Bangalore East",
      "29CCCCC2222C2Z2",
    ],
    [
    ...
    [
      ...
      "Sarah John",
      "9876543213",
      "info@globalmeds.demo",
      "Mumbai Central",
      "27EEEEE4444E4Z4",
    ],
    [
      "HealthFirst Suppliers",
      "Mohan Das",
      "9876543214",
      "supply@healthfirst.demo",
      "Delhi North",
      "07FFFFF5555F5Z5",
    ],
    [
  ...
  for (var _a = 0, suppliers_1 = suppliers; _a < suppliers_1.length; _a++) {
    ...
    var res = insertSupplier.run.apply(
      insertSupplier,
      __spreadArray(__spreadArray([], s, false), [now, now], false),
    );
  ...
  for (var i = 0; i < productsData.length; i++) {
    ...
    var res = insertProduct.run.apply(
      insertProduct,
      __spreadArray(__spreadArray([], p, false), [now, now], false),
    );
  ...
  for (var i = 0; i < 25; i++) {
    ...
    var res = insertPurchase.run(
      supplierId,
      "INV-PUR-".concat(1000 + i),
      purchaseDate,
      0,
      "COMPLETED",
      purchaseDate,
    ...
    for (var j = 0; j < numItems; j++) {
      ...
      var piRes = insertPurchaseItem.run(
        purchaseId,
        productId,
        batchNo,
        expiry,
      ...
      purchaseItemsMap.set(piRes.lastInsertRowid, {
      ...
      var ibRes = insertInventoryBatch.run(
        productId,
        batchNo,
        expiry,
        qty,
    ...
    db.prepare("UPDATE purchases SET total_amount = ? WHERE id = ?").run(
      totalAmount,
      purchaseId,
    );
  }
  ...
  var customerIds = [];
  for (
    var _b = 0, customersData_1 = customersData;
    _b < customersData_1.length;
    _b++
  ) {
    var c = customersData_1[_b];
    var res = insertCustomer.run.apply(
      insertCustomer,
      __spreadArray(__spreadArray([], c, false), [now, now], false),
    );
  ...
  for (var i = 0; i < 15; i++) {
    ...
    var res = insertPrescription.run(
      custId,
      pDate,
      "Dr. Fictional ".concat(i),
      "MCI-DEMO-".concat(100 + i),
      "RX-DEMO-".concat(100 + i),
      "Viral fever and cough",
      pDate,
      pDate,
    );
  ...
  var getBatchesForProduct = function (pid) {
    return Array.from(batchesMap.values())
      .filter(function (b) {
        return b.product_id === pid && b.quantity > 0;
      })
      .sort(function (a, b) {
  ...
  for (var i = 0; i < 40; i++) {
    ...
    if (i % 5 === 0) {
      ...
      custId = db
        .prepare("SELECT customer_id FROM prescriptions WHERE id = ?")
        .get(presId).customer_id;
    }
    var payMethod = ["Cash", "Card", "UPI"][i % 3];
    // Draft sale first
    var sRes = insertSale.run(
      "DEMO-".concat(invoiceNum++),
      sDate,
      0,
      0,
      0,
    ...
    for (var j = 0; j < numItems; j++) {
      ...
      for (
        var _c = 0, availableBatches_1 = availableBatches;
        _c < availableBatches_1.length;
        _c++
      ) {
    ...
    db.prepare(
      "UPDATE sales SET subtotal = ?, tax_amount = ?, total_amount = ? WHERE id = ?",
    ).run(subtotal, taxAmount, totalAmount, saleId);
    salesMap.set(saleId, {
  ...
  for (var i = 0; i < 8; i++) {
    ...
    var srRes = insertSalesReturn.run(
      sItem.saleId,
      "SR-".concat(returnNum++),
      rDate,
      refundTotal,
  ...
  var _loop_2 = function (i) {
    ...
    var batch = Array.from(batchesMap.values()).find(function (b) {
      return (
        b.product_id === pItem.productId && b.batch_number === pItem.batchNo
      );
    });
    if (batch && batch.quantity >= returnQty) {
      var lineTotal = returnQty * pItem.pp;
      var prRes = insertPurchaseReturn.run(
        pItem.purchaseId,
        "PR-".concat(prNum++),
        prDate,
        lineTotal,
  ...
  var activeBatches = Array.from(batchesMap.values()).filter(function (b) {
...
report += "- Foreign Key Check: ".concat(
  fks.length === 0 ? "Passed (0 errors)" : "FAILED",
  "\n",
);
report += "- Applied Migrations: ".concat(
  migrationsCount.c,
  " (Expected 10)\n\n",
);
...
var totalRefunds = db
  .prepare("SELECT sum(refund_amount) as sum FROM sales_returns")
  .get();
...
report += "- Net Sales: \u20B9 ".concat(
  ((grossSum - refundSum) / 100).toFixed(2),
  "\n",
);
report += "\n## Inventory Summary (Derived)\n";
var totalInv = db
  .prepare("SELECT sum(quantity) as sum FROM inventory_batches")
  .get();
