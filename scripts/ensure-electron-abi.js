...
function checkAndRebuild() {
  try {
    ...
    console.log('[ABI Check] better-sqlite3 is already compiled for Electron. Skipping rebuild.');
    process.exit(0);
  } catch (e) {
    console.log('[ABI Check] better-sqlite3 is NOT compiled for Electron. Rebuilding...');
    rebuild();
  }
}
...
function rebuild() {
  try {
    ...
    console.error('An existing Electron process is likely locking the file better_sqlite3.node.');
    console.error('Please close NovoPharma or kill lingering electron.exe processes in Task Manager.');
    console.error('---------------------------------------------------------\n');
    process.exit(1);
  }
}
