...
This layer handles the _User_ identity. It does not independently prove the _Installation_ identity. A subsequent Phase will bridge the two by orchestrating a cryptographic challenge/response protocol where the backend demands an Ed25519 signature corroborating the installation's physical authorization.
